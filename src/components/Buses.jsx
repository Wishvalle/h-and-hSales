...
export const Buses = () => {
    ...
    const mapper = {
      ...
      cantidad_de_asientos: 'Cantidad de Asientos', 
    };
    ...
    ? selectedBrand.models.filter(model =>
        model.motor === selectedMotor &&
        (selectedCategory === 'todos' || model.category === selectedCategory)
      )
    : [];
    ...
    return (
      ...
      <div className='container mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='text-center mb-12'>
          ...
          <p className='text-slate-800 mt-2'>1. Seleccione una marca.</p>
        </div>
        ...
        <div className='flex justify-center flex-wrap gap-4 mb-12'>
          {busBrands.map((brand) => (
            <button
              ...
              className={`p-2 bg-slate-700 rounded-lg transition-all duration-300 transform hover:scale-110 hover:bg-slate-600 focus:outline-none ${selectedBrand?.id === brand.id ? 'ring-2 ring-blue-500 scale-110' : ''}`}
            >
                  ...
                  <div className='flex flex-col md:flex-row gap-8'>
                    <aside className='md:w-1/4 lg:w-1/5' >
                      ...
                      <div className='flex flex-row flex-wrap md:flex-col gap-2'>
                        ...
                        <button onClick={() => handleMotorClick('combustion')} className={`w-full text-left p-3 rounded-md font-semibold transition-colors ${selectedMotor === 'combustion' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>Combustión</button>
                        <button onClick={() => handleMotorClick('electrico')} className={`w-full text-left p-3 rounded-md font-semibold transition-colors ${selectedMotor === 'electrico' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>Eléctrico</button>
                      ...
                      <div className='flex flex-row flex-wrap md:flex-col gap-2'>
                        {Object.entries(categoryOptions[selectedBrand.id][selectedMotor]).map(([key, value]) => (
                          <button
                            key={key}
                            onClick={() => setSelectedCategory(key)}
                            className={`w-full text-left p-3 rounded-md transition-colors duration-200 ${selectedCategory === key ? 'bg-blue-600 text-white font-bold' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
                          >
                            {value}
                          </button>
                        ))}
                      ...
                      {filteredModels.length > 0 ? (
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
                          {filteredModels.map((model) => (
                            <div key={model.id} className='bg-slate-900 rounded-xl overflow-hidden shadow-lg flex flex-col transform transition-transform duration-300 hover:translate-y-[-5px]'>
                              ...
                              <div className='p-6 flex flex-col'>
                                ...
                                <p className='text-slate-400 mb-4 flex-grow'>{model.description}</p>
                                ...
                                <div className='flex justify-center mt-4'>
                                  <button onClick={() => handleOpenModal(model)} className='px-6 py-2 rounded-md font-semibold transition-colors bg-blue-600 text-white hover:bg-blue-500'>Ver más detalles</button>
